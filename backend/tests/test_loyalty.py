...
from tests.test_order_service import seed_order_dependencies
...
def make_order(session, customer_id, product_id, key, *, payment=OrderStatus.PAID, fulfillment=FulfillmentStatus.NEW, quantity=1):
    ...
    order=Order(customer_user_id=customer_id,idempotency_key=key,request_fingerprint=(key[0]*64),public_access_token=f"token-{key}",status=payment,fulfillment_status=fulfillment,guest_name="Loyal Customer",guest_email=f"{key}@example.com",guest_phone="+15195550100",requested_pickup_at=now+timedelta(minutes=20),business_timezone="America/Toronto",currency="CAD",subtotal_cents=500*quantity,tax_cents=65*quantity,tax_name="HST",tax_rate_millionths=1_300_000,total_cents=565*quantity,version=1,expires_at=now+timedelta(hours=1),created_at=now,updated_at=now,items=[OrderItem(source_product_id=product_id,product_slug="drink",product_name="Drink snapshot",base_unit_price_cents=500,unit_price_cents=500,quantity=quantity,line_subtotal_cents=500*quantity,sort_order=0)])
...
def loyalty_db(postgresql_url):
    ...
    with Session(engine) as session:
        seed_order_dependencies(session);first_product=session.scalar(select(Product).order_by(Product.id));second_product=Product(category_id=first_product.category_id,slug="sandwich",name="Sandwich",description="Nonqualifying food",base_price_cents=900,is_published=True,is_featured=False,sort_order=1);session.add(second_product);org=Organization(slug=f"loyalty-{uuid4()}",name="Loyalty Test");application=JdsApplication(key=f"loyalty-{uuid4()}",name="Loyalty App");customer=JdsUser(primary_email=f"customer-{uuid4()}@example.com",display_name="Customer",status="active",credential_state="active");actor=JdsUser(primary_email=f"owner-{uuid4()}@example.com",display_name="Owner",status="active",credential_state="active");session.add_all([org,application,customer,actor]);session.flush();role=Role(application_id=application.id,key="customer",name="Customer");session.add(role);session.flush();session.add(Membership(organization_id=org.id,application_id=application.id,user_id=customer.id,role_id=role.id,status="active",joined_at=datetime.now(timezone.utc)));products=[first_product,second_product];program=LoyaltyProgram(organization_id=org.id,slug="coffee-tea",name="Coffee & Tea Loyalty",description="Buy 6, get your 7th free.",enabled=True,stamps_required=6,reward_description="Free drink",earning_rule="one_per_completed_qualifying_order",reward_type="free_qualifying_product");second=LoyaltyProgram(organization_id=org.id,slug="future",name="Future Program",description="Structural test",enabled=True,stamps_required=3,reward_description="Future reward",earning_rule="one_per_completed_qualifying_order",reward_type="free_qualifying_product");session.add_all([program,second]);session.flush();session.add_all([LoyaltyProgramProduct(loyalty_program_id=program.id,product_id=products[0].id,earning_eligible=True,reward_eligible=True),LoyaltyProgramProduct(loyalty_program_id=second.id,product_id=products[1].id,earning_eligible=True,reward_eligible=True)]);session.commit();ids=(org.id,customer.id,actor.id,products[0].id,products[1].id,program.id,second.id)
    yield engine,ids
    engine.dispose()
...
def test_nonqualifying_unpaid_failed_cancelled_and_disabled_do_not_earn(loyalty_db):
    ...
    with Session(engine) as session:
        ...
        for order in (nonqualifying,unpaid,failed,cancelled): assert loyalty.award_completed_order(order.id)==0
        session.get(LoyaltyProgram,program_id).enabled=False;disabled=make_order(session,customer,eligible,"disabled",fulfillment=FulfillmentStatus.COMPLETED);session.flush();assert loyalty.award_completed_order(disabled.id)==0
        session.get(LoyaltyProgram,program_id).enabled=True;reenabled=make_order(session,customer,eligible,"reenabled",fulfillment=FulfillmentStatus.COMPLETED);session.flush();assert loyalty.award_completed_order(reenabled.id)==1
...
def test_six_orders_issue_reward_without_off_by_one_and_programs_are_independent(loyalty_db):
    ...
    with Session(engine) as session:
        ...
        for index in range(6):
            order=make_order(session,customer,eligible,f"earned-{index}",fulfillment=FulfillmentStatus.COMPLETED);session.flush();assert loyalty.award_completed_order(order.id)==1
...
def test_eligibility_changes_only_future_orders_and_manual_adjustment_is_audited(loyalty_db):
    engine,(_,customer,actor,eligible,other,program_id,_)=loyalty_db
    with Session(engine) as session:
        loyalty=LoyaltyService(session);first=make_order(session,customer,eligible,"before-change",fulfillment=FulfillmentStatus.COMPLETED);session.flush();loyalty.award_completed_order(first.id);link=session.scalar(select(LoyaltyProgramProduct).where(LoyaltyProgramProduct.loyalty_program_id==program_id,LoyaltyProgramProduct.product_id==eligible));session.delete(link);session.add(LoyaltyProgramProduct(loyalty_program_id=program_id,product_id=other,earning_eligible=True,reward_eligible=True));session.flush();second=make_order(session,customer,eligible,"after-change",fulfillment=FulfillmentStatus.COMPLETED);session.flush();assert loyalty.award_completed_order(second.id)==0
...
@pytest.mark.postgresql
def test_ambiguous_organization_membership_fails_closed(loyalty_db):
    ...
    with Session(engine) as session:
        ...
        assert LoyaltyService(session).award_completed_order(order.id)==0
        assert session.scalar(select(func.count()).select_from(CustomerLoyaltyEvent).where(CustomerLoyaltyEvent.loyalty_program_id==program_id))==0
...
def test_concurrent_completion_requests_cannot_duplicate_stamp(loyalty_db):
    ...
    def complete():
        with Session(engine) as session:
            barrier.wait()
            try: OwnerOrderService(session).transition(order_id,target=FulfillmentStatus.COMPLETED,expected_version=1,now=datetime.now(timezone.utc))

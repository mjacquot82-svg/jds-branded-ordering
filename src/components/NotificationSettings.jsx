...
export default function NotificationSettings({csrfToken}){
 const {value:tenant}=useTenant();const businessName=tenant.business?.displayName||tenant.design?.displayName||"your café";
 ...
 return <section id="notifications" className="content-block notification-settings"><div className="account-card"><span className="account-avatar"><Bell size={24}/></span><div><h2>Café notifications</h2><p>Get today’s Lunch Special and occasional updates from {businessName}.</p></div></div>
  {state.error?<p className="form-status" role="alert">{state.error}</p>:null}
  {unavailable?<p>Café notifications are not available yet.</p>:support.needsInstall?<p>On iPhone or iPad, add {businessName} to your Home Screen, then open it there to enable notifications.</p>:!support.supported?<p>This browser doesn’t support café notifications.</p>:support.permission==="denied"?<p>Notifications are blocked in your browser settings. You can allow them there and return to this page.</p>:<>

...
## M3 status (starter image library)

- 26 of 70 archetypes have artwork: original, procedurally drawn **illustrated placeholders** (`artStyle: "illustrated-placeholder"` in the owner list API), generated by `backend/scripts/generate_starter_media.py`. They are not photographs and are labelled as placeholders in the picker. The other 44 archetypes stay hidden until art exists.
- The Harbor & Hearth demo starter assigns a starter image to all 9 of its products by reference (`starter:cafe-restaurant/<key>@1`); no `media_assets` rows are created and nothing counts against the prospect photo allowance.
- Owner previews resolve starter references and tenant uploads without a storefront hostname (`/api/v1/storefront/starter-media/...` and `/api/v1/owner/media/{id}/content`).
- The picker shows only available art, friendly category names, suggestions for the product name, a live product-card preview, a source badge (starter vs. your photo), and an explicit “Image chosen — not saved yet” prompt with a Save button.
- Owners can free photo allowance with “Delete unused photo” (product-purpose uploads not used by any product or the draft design). Nothing is deleted automatically.
- Production still needs reviewed, JDS-owned or licensed photography following the generation specification above.
